...
| `./web-app/src/views/ShowDetailView.vue` | Hauptansicht einer Show mit Kanaltabelle und Editoren; read-only-Overlay bei fremdem Schreib-Lock (leitet Scroll-Events an den Content weiter), Übernahme-Anfrage-Dialog; fixierte Statusanzeige (Lade-Spinner/Erfolg/Fehler) beim Kreisliste-Scan und beim Einleuchtplan-PDF-Scan (je eigener Block + eigene Vorschau-Dialog-Instanz). |
...
| `./web-app/src/components/show/BarVisualization.vue` | Visualisierung einer Zugstange/Traverse/Punktzug innerhalb ZugstangenView: Skala mit Ticks, Kanal-Marker (Klick/Drag), Bemaßungsrichtung (Mitte/Links/Rechts via `scale_origin`) nur für Anzeige-Label, gespeicherte Fixture-Position bleibt immer mitte-zentriert. |
